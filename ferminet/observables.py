...
def make_dipole(
  ...
  def dipole_estimator(
    ...
    del state, params  # only included for consistency in the interface
    # the dipole moment is trivial - it's just the expected position of an
    # electron, or the center of mass of the electron density
    return jnp.sum(jnp.reshape(data.positions, (-1, 3)), axis=0)
...
def make_density_matrix(
  ...
  if ckpt_state is not None:
    ...
    data_shape = (jax.local_device_count(), device_batch_size)
